...
import utils.metrics as eval_metrics
import utils.eval_tasks as eval_tasks
import utils.base as eval_base
from utils.eval_utils import positional_deprecated, run_task_tests

import numpy as np
import transformers


def simple_evluate(args):
    ...
    assert args.tasks != [], "No tasks specified"
    lm = LLM(args).model
    no_cache = True

    if not no_cache:
        lm = eval_base.CachingLM(lm, f"lm_cache/{args.model}")

    task_dict = eval_tasks.get_task_dict(args.tasks)
    ...
    results["config"] = {
        ...
        "limit": args.limit,
        "bootstrap_iters": 100000,
        "description_dict": args.description_dict,
    }
...
def evaluate(
    ...
    for reqtype, reqs in requests.items():
        ...
        for resp, (i, task_name, doc, doc_id) in zip(resps, requests_origin[reqtype]):
            ...
            if write_out:
                ...
                if isinstance(task, eval_base.MultipleChoiceTask):
    ...
    for (task_name, metric), items in vals.items():
        ...
        stderr = eval_metrics.stderr_for_metric(
